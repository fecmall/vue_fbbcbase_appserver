<template>
    <div class="content">
        <div class="product_page">
            <div class="product_view">
                ...
                <div class="product_info">
                    ...
                    <div class="price_info" v-if="product.price_info && product.price_info.special_price && productSpecialPrice">
                    ...
                    <div class="price_info" v-else-if="product.price_info && product.price_info.price && product.price_info.price.value" >
        ...
        <nav class="bar bar-tab">
            <div class="product_bottom_bar1"  style="width:45%;display:inline-block">
                <a  class="tab-item external" :href="servicesPhone" >
                    <span class="icon icon-phone"></span>
                    <span class="tab-label">{{ $t("message.services") }}</span>
                </a>
              
            <router-link to="/"  class="tab-item external" :class="isHomeActive ? 'active' : ''  ">
                <span class="icon icon-home"></span>
                <span class="tab-label">{{ $t("message.shop") }}</span>
            </router-link>
            ...
            <div class="product_bottom_bar2" style="width:54%;display:inline-block">
                ...
                <a @click="OpenAddToCart('buy-now')" href="javascript:void(0)"  class="tab-item external  buy-now" :class="isCartActive ? 'active' : ''  ">
                    {{ $t("message.buy_now") }}
                </a>
            </div>
        ...
        <div @click="closeBackGray()" class="back-gray" :style="bk_is_display? 'display:block' : 'display:none' ">
        
        </div>
        
        <div  class="add-cart-show-img product_info" :style="bk_is_display? 'display:block' : 'display:none' ">
            <div class="close_popup">
                <a @click="closePopup()" href="javascript:void(0)" class="closeAddToCart">×</a>
            </div>
          
            <img class="lazy" :src="main_img" alt="" style=''>
            <div class="price_info" v-if="product.price_info && product.price_info.special_price && productSpecialPrice">
                <div class="special_price special_active">
                    {{product.price_info.special_price.symbol}}
                    {{productSpecialPrice}}
        ...
        <div class="add_cart_confirm"   :style="bk_is_display? 'display:block' : 'display:none' ">
            <a @click="addProductToCart()" external href="javascript:void(0)" id="js_registBtn" class=" addProductToCart">
                <span>
                    <i></i>
                    {{ $t("message.add_to_cart_confirm") }}
                </span>
            </a>
        </div>
...
export default {
    ...
    data () {
        return {
            ...
            getCartActiveCountUrl: root + '/checkout/cart/activecount' ,
            refer_url: '',
            servicesPhone: '',
            is_buy_now: false
    ...
    computed: {
        ...
        productSpecialPrice: function(){
            if(this.product.price_info && this.product.price_info.special_price){
                ...
                return parseFloat(specialPrice) + parseFloat(this.custom_option_add_price);
            }
        },
        productPrice: function(){
            if(this.product.price_info && this.product.price_info.price){
                var price = this.product.price_info.price.value; 
                return parseFloat(price) + parseFloat(this.custom_option_add_price);
            }
        }
    },
    created: function(){
        this.fetchProduct();
        this.getCartActiveCount();
    },
    watch: {
        // 如果路由有变化，会再次执行该方法
    ...
    methods:{
        ...
        getCartActiveCount: function(){
            var self = this;
            $.ajax({
                url: self.getCartActiveCountUrl,
                async: true,
                timeout: 120000,
                type: 'get',
        ...
        closePopup: function(){
            ...
            this.bk_is_display = false;
        },
        OpenAddToCart: function(type){
            $.popup('.popup-about');
            this.bk_is_display = true;
            if (type == "buy-now") {
                this.is_buy_now = true;
            }
        },
        
        closeBackGray: function(){
            this.closePopup();
        },
            ...
            if(this.custom_option_all_select && selected_attr && custom_option){
                ...
                for(var x in custom_option){
                    if(x){
                        ...
                        if(same == 1){
                            this.custom_option_add_price = parseFloat(option['price']);
        ...
        addProductFavorite: function(){
            ...
            $.ajax({
                ...
                success:function(reponseData, textStatus,request){ 
                    ...
                    if(reponseData.code == 200){
                        $.toast(self.$i18n.t("message.favorite_success"));
                        ...
                        $.toast(self.$i18n.t("message.favorite_fail"));
                    }
            ...
            $.ajax({
                ...
                success:function(reponseData, textStatus,request){ 
                    ...
                    if(reponseData.code == 1100003){
                        self.setLoginSuccessRedirectUrl('/catalog/product/' + self.product._id);
                        self.$router.push('/customer/account/login');
                        return;
            ...
            $.ajax({
                ...
                success:function(reponseData, textStatus,request){
                    ...
                    if(reponseData.code == 200){
                        ...
                        self.servicesPhone = "tel:" + reponseData.data.servicesPhone;
                        ...
                        self.main_img = product.main_img;
                        ...
                        $.toast(self.$i18n.t("message.product_is_off_the_shelf"));
                    }else{
                        $.toast(self.$i18n.t("message.get_product_info_error"));
                    }
...
.add-cart-show-img .price_info{
    margin-left: 5rem;
    margin-top: 0.8rem;
}

.product_info .price.no-special {
    color: #c00;
    font-size: 25px;
}
