...
import SuiExtendCity from './assets/js/sm-city-picker.min.js'
Vue.use(SuiExtendCity)
...
var store_uuid = decodeURIComponent((new RegExp('[?|&]' + 'store_uuid' + '=' + '([^&;]+?)(&|#|;|$)').exec(location.href) || [, ""])[1].replace(/\+/g, '%20')) || null ;
if (store_uuid) {
    window.localStorage.setItem("store-uuid", store_uuid, 12*30*24*1000);
    console.log('set store-uuid:' + store_uuid);
}

Vue.prototype.getRequestHeader = function (){
    var headers = {};
    var store_uuid_item = window.localStorage.getItem("store-uuid");
    if(store_uuid_item){
        console.log('fecshop store uuid ######' + store_uuid_item);
        headers['fecshop-store-uuid'] = store_uuid_item;
    }
